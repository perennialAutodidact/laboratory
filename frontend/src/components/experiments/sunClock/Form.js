...
const Form = () => {
  ...
  const { showForm, coordsSearchResults } = useSelector(
    (state) => state.sunClock
  );
  ...
  // useEffect(()=>{
  //   setShowSearchResults(coordsSearchResults.length > 0)
  // }, [coordsSearchResults])

  // clear state and city fields based on country value
  useEffect(() => {
  ...
  // enable/disable city/state fields based on country value
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {showSearchResults && (
        <>
        {/* <h1>{JSON.stringify(coordsSearchResults)}</h1> */}
        <SearchResults results={coordsSearchResults} />
        </>
      )}
